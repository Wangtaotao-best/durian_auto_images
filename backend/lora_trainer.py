...
class LoRATrainer:
    ...
    def _check_cuda_architecture(self):
        if self.device.type != "cuda":
            return

        if not torch.cuda.is_available():
            raise RuntimeError(
                "CUDA 设备不可用，请检查 CUDA 驱动或将 device 设置为 'cpu'."
            )
        ...
        prop = torch.cuda.get_device_properties(cuda_device)
        current_arch = f"sm_{prop.major}{prop.minor}"

        if current_arch not in supported_archs:
            raise RuntimeError(
                f"当前 GPU {prop.name} (compute capability {current_arch}) 不受此 PyTorch 安装支持。"
                f" 已支持的 CUDA 架构: {supported_archs}."
                " 请安装支持 sm_120 的 PyTorch/CUDA 版本，或将 device 设置为 'cpu'。"
            )
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                # 将数据移到设备
                pixel_values = batch["pixel_values"].to(self.device).to(dtype=self.unet.dtype)
